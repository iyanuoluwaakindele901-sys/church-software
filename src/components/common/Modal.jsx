...
export function Modal({ modal, onClose }) {
  ...
  return (
    <div onClick={modal.type === 'choice' ? undefined : onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {modal.type === 'choice' && (
          <div style={{ display: 'grid', gap: '8px' }}>
            {(modal.options || []).map((option) => (
              <button key={option.label} onClick={() => { onClose(); option.onSelect(); }} style={{ padding: '10px 12px', background: option.primary ? '#d4a574' : 'rgba(255,255,255,0.08)', border: option.primary ? 'none' : '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', color: option.primary ? '#1a1a1a' : 'white', fontSize: '12px', fontWeight: '700', cursor: 'pointer', textAlign: 'left' }}>{option.label}</button>
            ))}
          </div>
        )}
        {modal.type !== 'choice' && <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
